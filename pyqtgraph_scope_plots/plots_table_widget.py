...
from .multi_plot_widget import MultiPlotWidget, DroppableMultiPlotWidget, LinkedMultiPlotWidget
...
class PlotsTableWidget(QSplitter):
    ...
    def _set_data_items(self, new_data_items: List[Tuple[str, QColor, 'MultiPlotWidget.PlotType']]) -> None:
    ...
    def _set_data(self, data: Mapping[str, Tuple[np.typing.ArrayLike, np.typing.ArrayLike]]) -> None:
        ...
        self._table.set_data({data_name: vals for data_name, vals in self._transformed_data.items()
                              if self._data_items.get(data_name, (None, MultiPlotWidget.PlotType.DEFAULT))[1] != MultiPlotWidget.PlotType.ENUM_WAVEFORM })
    ...
    def _write_csv(self, fileio: Union[TextIO, StringIO]) -> None:
        ...
        while True:  # iterate each row
            xs_at_index = [ordered_data_items[data_index][0][point_index] for data_index, point_index in enumerate(indices)
    ...
    def _save_csv_dialog(self) -> None:
        ...
        if self._data is None:
            return
        filename, filter = QFileDialog.getSaveFileName(self, f"Save Data",
                                                       "", "CSV (*.csv)")
        if not filename:
            return
