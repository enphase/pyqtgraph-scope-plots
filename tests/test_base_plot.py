...
def test_plot_save(qtbot: QtBot, plot: PlotsTableWidget) -> None:
    qtbot.waitUntil(
        ...
        == [PlotWidgetModel(data_items=["0"]), PlotWidgetModel(data_items=["1"]), PlotWidgetModel(data_items=["2"])]
    )

    plot._plots._merge_data_into_item(["0"], 1)  # merge
    qtbot.waitUntil(
        lambda: cast(MultiPlotStateModel, plot._plots._dump_model([])).plot_widgets
        == [PlotWidgetModel(data_items=["1", "0"]), PlotWidgetModel(data_items=["2"])]
    )

    plot._plots._merge_data_into_item(["2"], 0)  # merge
    qtbot.waitUntil(
        lambda: cast(MultiPlotStateModel, plot._plots._dump_model([])).plot_widgets
        == [PlotWidgetModel(data_items=["1", "0", "2"])]
    )
