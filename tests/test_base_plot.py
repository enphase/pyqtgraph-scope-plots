...
def test_plot_remove(qtbot: QtBot, plot: PlotsTableWidget) -> None:
    ...
    qtbot.waitUntil(lambda: len(
        cast(pg.PlotItem, cast(pg.PlotWidget, plot._plots.widget(0)).getPlotItem()).listDataItems()) == 1)

    plot._plots.remove_plot_items(['2'])  # delete the last plot, the empty plot should appear
    qtbot.waitUntil(lambda: len(
        cast(pg.PlotItem, cast(pg.PlotWidget, plot._plots.widget(0)).getPlotItem()).listDataItems()) == 0)
...
def test_linked_drag_cursor(qtbot: QtBot, plot: PlotsTableWidget) -> None:
    ...
    qtbot.waitUntil(lambda: plot_item(plot, 1).drag_cursor is not None and
                            plot_item(plot, 1).drag_cursor.pos().x() == 0.2)
...
2.0,0.0,0.5,0.5""".replace('\r', '').replace('\n', '')  # ignore newline format
    ...
    plot._set_data({  # more comprehensive missing data test
        '0': ([0, 2], [0.01, 0]),
        '1': ([0, 1], [0.25, 0.5]),
        '2': ([1, 2], [0.7, 0.6]),
    })
