...
def test_plot_merge(qtbot: QtBot, plots: DroppableMultiPlotWidget) -> None:
    ...
    # +1 is for the empty hover scatterpoints, *2 for line + scatter items
    assert len(cast(pg.PlotItem, cast(pg.PlotWidget, plots.widget(0)).getPlotItem()).listDataItems()) == 2 * 2 + 1
    assert len(cast(pg.PlotItem, cast(pg.PlotWidget, plots.widget(1)).getPlotItem()).listDataItems()) == 1 * 2 + 1
    ...
    assert len(cast(pg.PlotItem, cast(pg.PlotWidget, plots.widget(0)).getPlotItem()).listDataItems()) == 1 * 2 + 1
    assert len(cast(pg.PlotItem, cast(pg.PlotWidget, plots.widget(1)).getPlotItem()).listDataItems()) == 2 * 2 + 1
    assert table.rowCount() == 3  # signals table should not change
    assert table.item(0, table.COL_NAME).text() == "0"
    assert table.item(1, table.COL_NAME).text() == "1"
    assert table.item(2, table.COL_NAME).text() == "2"
    ...
    # +1 is for the empty hover scatterpoints, *2 for line + scatter items
    assert len(cast(pg.PlotItem, cast(pg.PlotWidget, plots.widget(0)).getPlotItem()).listDataItems()) == 3 * 2 + 1
    assert table.rowCount() == 3  # signals table should not change
    assert table.item(0, table.COL_NAME).text() == "0"
    assert table.item(1, table.COL_NAME).text() == "1"
    assert table.item(2, table.COL_NAME).text() == "2"
...
def test_plot_remove(qtbot: QtBot, plots: DroppableMultiPlotWidget) -> None:
    ...
    qtbot.waitUntil(
        lambda: len(cast(pg.PlotItem, cast(pg.PlotWidget, plots.widget(0)).getPlotItem()).listDataItems()) == 1 * 2 + 1
    )
