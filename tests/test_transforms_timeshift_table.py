...
def test_transform_x(qtbot:QtBot, transforms_table: TransformsSignalsTable) -> None:
    ...
    target = transforms_table.visualItemRect(not_none(transforms_table.item(0, transforms_table.COL_TRANSFORM))).center()
    ...
    target = transforms_table.visualItemRect(not_none(transforms_table.item(1, transforms_table.COL_TRANSFORM))).center()
    with mock.patch.object(QInputDialog, 'getText') as mock_input:
